@php
$service = config('market.payment.service');
if ($service) {
  $mid = "SR".config('market.payment.mid');
...
<form name="payment-form">
  ...
  <input type="hidden" name="site_cd"         value="{{ $mid }}">
  <input type="hidden" name="site_name"       value="{{ Config::get('app.name') }}">
  <input type="hidden" name="def_site_cd"     value="{{ $mid }}">
  <!--
...
휴대폰": "000010000000";
"신용카드": "100000000000";
간편결제": 100000000000  (payco_direct = Y, test Card: site_cd: S6729)
-->

    <input type="hidden" name="ordr_idxx"   value="{{$buyer->o_id}}"> <!-- 주문코드 -->
    <input type="hidden" name="good_name"   value="{{$display->products}}"><!-- 주문상품 -->
  ...
  -->
    ...
    <input type="hidden" name="bask_cntx" value="{{ count($items) }}">

    <!-- 장바구니 상품 상세 정보 (자바 스크립트 샘플(create_goodInfo()) 참고) -->
    <input type="hidden" name="good_info" value="{{$display->products}}">
...
{{-- 
    ...
    <input type="hidden" name="comm_free_mny"     value="0">        <!-- 비과세 금액 -->
--}}
...
{
  var frm = document.forms['payment-form'];
