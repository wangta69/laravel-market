...
@section('title', '상품문의')
@section('content')
@include('market::admin.layouts.main-top', ['path'=>['상품문의 및 리뷰', '상품문의']])

<div class="p-3 mb-4 bg-light rounded-3">
  <h2 class="fw-bold">상품문의</h2>
</div>
  ...
  <div class="card-body">
    ...
    <table class="table items">
      ...
      <tr user-attr-id="{{$item->id}}">
        ...
        <td class="text-center">
          <span data-bs-toggle="modal" data-bs-target="#qnaModal-{{$item->id}}">{{$item->title}}</span>
              ...
              <div class="modal-content">
                ...
                <form>
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary act-save-reply">답변 저장</button>
                  </div>
                </form>
              </div><!-- .modal-content -->
            </div><!-- .modal-dialog -->
        ...
        <td class="text-center">
          @if($item->reply) 
            <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#qnaModal-{{$item->id}}">답변완료</button> 
          @else 
            <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#qnaModal-{{$item->id}}">답변준비중</button> 
          @endif
        </td>
      </tr>
      @empty
