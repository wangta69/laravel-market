...
use App\Http\Controllers\Market\Payment\Lgxpay\Lgdacom\XPayClient;
...
class LgController extends Controller
{
  ...
  public function hashdata(Request $request) {
    ...
    */
    $LGD_OID                    = $request->LGD_OID;                //주문번호(상점정의 유니크한 주문번호를 입력하세요)
    $LGD_AMOUNT                 = $request->LGD_AMOUNT;             //결제금액("," 를 제외한 결제금액을 입력하세요)
    $LGD_TIMESTAMP              = $request->LGD_TIMESTAMP;          //타임스탬프
    $LGD_BUYER                  = $request->LGD_BUYER;              //구매자명
    $LGD_PRODUCTINFO            = $request->LGD_PRODUCTINFO;        //상품명
    $LGD_BUYEREMAIL             = $request->LGD_BUYEREMAIL;         //구매자 이메일
    $LGD_CUSTOM_FIRSTPAY        = $request->LGD_CUSTOM_FIRSTPAY;    //상점정의 초기결제수단
    $LGD_CUSTOM_SKIN            = 'red';                            //상점정의 결제창 스킨
    $LGD_CUSTOM_USABLEPAY       = $request->LGD_CUSTOM_USABLEPAY;   //디폴트 결제수단 (해당 필드를 보내지 않으면 결제수단 선택 UI 가 노출됩니다.)
    $LGD_WINDOW_VER             = '2.5';                            //결제창 버젼정보
    $LGD_WINDOW_TYPE            = 'iframe'; // $LGD_WINDOW_TYPE;                 //결제창 호출방식 (수정불가)
    $LGD_CUSTOM_SWITCHINGTYPE   = 'IFRAME'; // $LGD_CUSTOM_SWITCHINGTYPE;        //신용카드 카드사 인증 페이지 연동 방식 (수정불가)
    $LGD_CUSTOM_PROCESSTYPE     = 'TWOTR';                          //수정불가


    $CST_PLATFORM = $request->CST_PLATFORM;
    $CST_MID = $request->CST_MID;
    $LGD_MID = $request->LGD_MID; // (('test' == $CST_PLATFORM) ? 't' : '').$CST_MID;
    $LGD_RETURNURL = $request->LGD_RETURNURL;
    $LGD_VERSION = 'PHP_Non-ActiveX_Standard'; // $request->LGD_VERSION;
    $LGD_CASNOTEURL = $request->LGD_CASNOTEURL;
    /*
    *************************************************
    ...
    */
    $configPath = storage_path().'/app/market/payment/lgdacom';
    
    $xpay = new XPay($configPath, $CST_PLATFORM);
   
    // Mert Key 설정
    $xpay->set_config_value('t'.$LGD_MID, config('market.payment.mid'));
    $xpay->set_config_value($LGD_MID, config('market.payment.mid'));
